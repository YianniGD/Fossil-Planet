...
function draw() {
  if (!gameStarted) {
    background(102, 69, 44);
    fill(255);
    textSize(24);
    text("Fossils are buried deep within the earth.", width / 2, height / 2 - 40);
    textSize(16);
    text("Click to smack, hold and drag to brush.", width / 2, height / 2 - 10);
    text("Uncover large rocks, then tap them quickly to break them.", width / 2, height / 2 + 10);
    textSize(20);
    text("Click anywhere to begin digging.", width / 2, height / 2 + 50);
  ...
  push();
  if (shakeDuration > 0) {
    translate(random() * (shakeAmount * 2) - shakeAmount, random() * (shakeAmount * 2) - shakeAmount);
    shakeAmount *= 0.9;
    shakeDuration--;
  }
  ...
  if (gameWon) {
    cursor(ARROW);
    fill(255, 223, 100, 200);
    rect(0, 0, width, height);
    fill(0);
    textSize(64);
    text("You discovered the fossils!", width / 2, height / 2);
    textSize(24);
    text("Click anywhere to dig again.", width / 2, height / 2 + 60);
  } else {
    noCursor();
...
function initializeLayers() {
  ...
  for (let i = 0; i < NUM_FOSSILS; i++) {
    fossilLocations.push({
      x: random() * (width * (1 - padding * 2)) + width * padding,
      y: random() * (height * (1 - padding * 2)) + height * padding
    });
  }
  ...
  fossilLayer = createGraphics(width, height);
  drawFossil(fossilLayer);
  totalFossilPixels = countFossilPixels(fossilLayer);

  dirtRockColor = color(87, 57, 28);
  stoneRockColor = color(85, 80, 75);
  bedrockRockColor = color(40, 42, 45);
  dirtBaseColor = color(139, 90, 43);
  stoneBaseColor = color(120, 110, 100);
  bedrockBaseColor = color(55, 58, 64);

  dirtLayer = createGraphics(width, height);
  drawDirt(dirtLayer);
  stoneLayer = createGraphics(width, height);
  drawStone(stoneLayer);
  bedrockLayer = createGraphics(width, height);
...
function drawDirt(pg) {
  ...
  for (let i = 0; i < 300000; i++) {
    let x = random() * pg.width, y = random() * pg.height, n = noise(x * 0.01, y * 0.01);
...
function drawStone(pg) {
  ...
  for (let i = 0; i < 200000; i++) {
    let x = random() * pg.width, y = random() * pg.height, n = noise(x * 0.005, y * 0.005);
...
function drawHardRocks(pg, rockColor, numRocks, sizeMultiplier, rockArray) {
  for (let i = 0; i < numRocks; i++) {
    ...
    let rockW = (random() * (width * 0.15) + width * 0.1) * sizeMultiplier,
        rockH = (random() * (height * 0.15) + height * 0.1) * sizeMultiplier;
    let vertices = [], detail = 10;
    for (let j = 0; j < detail; j++) {
      let angle = map(j, 0, detail, 0, TWO_PI);
      let r = (rockW / 2) + (random() * (60 * sizeMultiplier) - 30 * sizeMultiplier);
      let x = rockX + cos(angle) * r,
          y = rockY + sin(angle) * (r * (rockH / rockW));
    ...
    for (let j=0; j<numSpeckles; j++) {
      ...
      pg.ellipse(x,y,random()*3+1*sizeMultiplier, random()*3+1*sizeMultiplier);
    }
...
function brushLayer(targetLayer, minFringeSize, maxFringeSize) {
  ...
  targetLayer.pop();
  for (let i = 0; i < 10; i++) {
    let offsetX = random() * (brushSize * 1.6) - brushSize * 0.8;
    let offsetY = random() * (brushSize * 1.6) - brushSize * 0.8;
    let grainSize = random() * (maxFringeSize - minFringeSize) + minFringeSize;
...
function managePopovers() {
  for (let i = popovers.length - 1; i >= 0; i--) {
    ...
    pt.lifespan--;
    pt.y -= 0.5;
    let alpha = map(pt.lifespan, 0, pt.initialLife / 2, 0, 255);
    textSize(16);
    stroke(0, alpha);
    strokeWeight(3);
    fill(255, 255, 220, alpha);
    text(pt.text, pt.x, pt.y);
    noStroke();
...
function getOutskirtPosition() {
    ...
    let pos = { x: 0, y: 0 };
    switch (edge) {
      case 0: pos.x=random() * (width - 2 * paddingX) + paddingX; pos.y=random() * (paddingY * 0.5) + paddingY * 0.5; break;
      case 1: pos.x=random() * (paddingX * 0.5) + (width - paddingX); pos.y=random() * (height - 2 * paddingY) + paddingY; break;
      case 2: pos.x=random() * (width - 2 * paddingX) + paddingX; pos.y=random() * (paddingY * 0.5) + (height - paddingY); break;
      case 3: pos.x=random() * (paddingX * 0.5) + paddingX * 0.5; pos.y=random() * (height - 2 * paddingY) + paddingY; break;
    }
    return pos;
}
...
function triggerPopover() {
  if (millis() - lastPopoverTime > popoverCooldown) {
    ...
    popovers.push({
      text: encouragementPhrases[floor(random() * encouragementPhrases.length)],
